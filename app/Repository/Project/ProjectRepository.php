...
use Tinyissue\Contracts\Repository\ProjectRepository as ProjectContract;
...
class ProjectRepository extends Repository implements ProjectContract
{
    ...
    {
        return $this->getBy('status', Project::STATUS_OPEN);
    }
    ...
    {
        return $this->getBy('private', Project::PRIVATE_NO);
    }
    ...
    {
        $users = User::where('deleted', '=', User::NOT_DELETED_USERS);

        if ($this->hasModel()) {
            $userIds = $this->model->users()->lists('user_id')->all();
            $users->whereNotIn('id', $userIds);
        }

        return $users->get(['fullname', 'id'])->lists('fullname', 'id')->all();
    }
     ...
     */
    public function getListIssues($status = Project\Issue::STATUS_OPEN, array $filter = [])
    {
     ...
     * @return \Illuminate\Database\Eloquent\Collection
     */
    public function getListAssignedOrCreatedIssues(User $user)
    {
     ...
     */
    public function getProjectsWidthIssues($status = Project::STATUS_OPEN, $private = Project::PRIVATE_NO)
    {
        $query = $this
            ->where('status', '=', $status)
            ->orderBy('name');
    ...
    public function getKanbanTagsForUser(User $user)
    {
        $tags = $this->kanbanTags()->withRoleLimitForUser($user);
     ...
     * @return mixed
     */
    public function getIssuesGroupByTags($tagIds)
    {
        $issues = $this->issues()
            ->with('user', 'tags')
            ->where('status', '=', Project\Issue::STATUS_OPEN)
    ...
    {
        return $this->users()->where('users.role_id', '>', 1)->where('users.deleted', '=', User::NOT_DELETED_USERS)->get();
    }
